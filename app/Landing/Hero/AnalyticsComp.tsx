...
const AnalyticsComp = ({ expenses }: { expenses: Expense[] }) => {
  const monthlyTrend = buildMonthlyTrend(expenses);
  const categorySummary = summarizeCategories(expenses).map((item, index) => ({
    ...item,
  ...
  return (
    <section id="analytics" className="section-enter py-5" style={{ animationDelay: "100ms" }}>
      ...
      <div className="mt-4 grid gap-5 md:[grid-template-columns:1fr_1px_1fr]">
        <div className="pr-0 md:pr-4">
          ...
          <h3 className="headline mb-3 text-[16px]">Spending Over Six Months</h3>
          <Barchart monthlyData={monthlyTrend} />
          <div className="mt-2 flex gap-4">
            <div className="flex items-center gap-1.5">
              <div className="h-2.5 w-2.5 bg-[var(--color-ink)]" />
              <span className="text-[10px] text-[var(--color-ink-mid)]" style={{ fontFamily: "var(--font-jetbrains)" }}>
                Previous months
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="h-2.5 w-2.5 bg-[var(--color-accent)]" />
              <span className="text-[10px] text-[var(--color-accent)]" style={{ fontFamily: "var(--font-jetbrains)" }}>
                Current month
              </span>
            </div>
          </div>
              ...
              <div className="mt-3">
                {categorySummary.map((cat) => (
                  <div key={cat.name} className="mb-1.5 flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <div className="h-2.5 w-2.5 flex-shrink-0" style={{ background: cat.color }} />
                      <span className="text-[12px] text-[var(--color-ink-mid)]" style={{ fontFamily: "var(--font-jetbrains)" }}>
                        {cat.name}
                      </span>
                    </div>
                    <span
